...
class AliceGPT(loader.Module):
    ...
    async def alice(self, message):
        ...
        async with message.client.conversation(bot1[0]) as conv:
            ...
            if "Начинаю творить, вернусь через несколько секунд" or "Рисую, через несколько секунд будет готово" or "Дайте мне несколько секунд" in response1.text:
             ...
             await utils.answer(message, {response2.object})
